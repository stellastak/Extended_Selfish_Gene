...
for problem_size in tqdm([10], desc="PROBLEM SIZE", position=0, leave=None, ncols=0):
    fitness_function = sgx.fitness.FitnessFunction(lambda i: i.count('X'),
                                               best_fitness=problem_size,
                                               type_=sgx.fitness.Scalar)
    experiments = product(tqdm(np.linspace(1e-6, 0.2, 5), leave=False), range(5))
    for lr, _ in tqdm(list(experiments), position=1, leave=None, ncols=0):
        ...
        genome = sgx.t.Genome(
            [sgx.allele.Categorical("abcX", learning_rate=lr)] + [sgx.allele.Categorical("01X", learning_rate=lr) for _
                                                                  in range(problem_size)])
...
for ind in results.index:
    if index_of_last_row-1 != ind:
        if results['Learning Rate'][ind] == results['Learning Rate'][ind+1]:
...
results1.to_sql(name='base', con='sqlite:///multi-values_learning_rate.sqlite3', if_exists='replace')