...
for problem_size in tqdm([5,10,15,20], desc="PROBLEM SIZE", position=0, leave=None, ncols=0):
    ...
    for lr, _ in tqdm(list(experiments), position=1, leave=None, ncols=0):
        ...
        genome = sgx.t.Genome(
            [sgx.allele.Categorical("abcX", learning_rate=lr)] + [sgx.allele.Categorical("01X", learning_rate=lr) for _ in range(problem_size)])
...
results1.to_sql(name='base', con='sqlite:///multi-values_size.sqlite3', if_exists='replace')